...
int getMaxProfitBrute(vector<int> v) {
...
int maxProfitOptimal(vector<int>& v) {
    int n = v.size();
    // If there is only one price, no transaction can be made, so the profit is 0.
    if (n == 1)
        return 0;

    int maxProfit = 0;  // Variable to store the maximum profit found so far.
    int minimum = v[0]; // Initialize minimum price to the first day's price.
    ...
    for (int i = 1; i < n; i++) {
        ...
        maxProfit = max(maxProfit, v[i] - minimum);
        
        // Update minimum to keep track of the lowest price seen so far.
...
void solve() {
    ...
    auto brute = getMaxProfitBrute(v);
    debug(brute);
