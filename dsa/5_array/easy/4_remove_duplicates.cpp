...
vector<int> remove_duplicates(vector<int> v)
{
    ...
    for(auto i: v)
    {
        if(result.size()==0 or result[result.size()-1] != i){
            result.push_back(i);
        }
    }
    return result;
}
int remove_duplicates_using_set(int arr[],int n)
{
    // if there is only one element return the element
    if(n<=1){
        return n;
...
int remove_duplicates_using_2pointers(int arr[],int n)
{
     // if there is only one element return the element
    if(n<=1){
        return n;
...
void solve() {
    vector<int> v = {1,1,1,2,2,3,3,3,4,5};
    auto unique = remove_duplicates(v);
    debug(unique);

    // int max_element_by_sorting = usingSorting(arr,arr_size);
    // debug(max_element_by_sorting);

    int arr1[] = {1,1,1,2,2,3,3,3,4,5};
    int arr_size1 = end(arr1)-begin(arr1);
    int k = remove_duplicates_using_set(arr1, arr_size1);
    for(int i=0;i<k;i++)
    {
        cout<<arr1[i]<<" ";
    }
    ...
    int arr2[] = {1,1,1,2,2,3,3,3,4,5};
    int arr_size2 = end(arr2)-begin(arr2);
    int k2 = remove_duplicates_using_2pointers(arr2, arr_size2);
    cout<<"\n"<<"2pointer: ";
    for(int i=0;i<k2;i++)
    {
        cout<<arr2[i]<<" ";
