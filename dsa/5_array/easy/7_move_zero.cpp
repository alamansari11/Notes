...
void move_zero_brute(vector<int> &a) {
    //temporary array:
    vector<int> temp;
    ...
    for (int i = nz; i < a.size(); i++) {
        a[i] = 0;
    }
}

void move_zero_optimal(vector<int>& nums) {
        int firstPointer = 0;
        for (int secondPointer = 0; secondPointer < nums.size();secondPointer++) {
            if (nums[secondPointer] != 0) {
                if (firstPointer != secondPointer) {
                    swap(nums[firstPointer], nums[secondPointer]);
                }
                firstPointer++;
            }
        }
...
void solve() {
    ...
    vector<int> v1 = {0,0,3,0,5};
    move_zero_optimal(v1);
    debug(v1);
