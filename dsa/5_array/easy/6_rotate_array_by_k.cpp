...
void right_rotate_optimal(int arr[],int n,int k)
{
    ...
    k = k % n;
    if (k > n) return;
    //just reverse last k elements 
    reverse(arr+n-k,arr+n);
    //reverse the remaining the element
    reverse(arr,arr+n-k);
    //reverse the whole array
    reverse(arr,arr+n);
}
void right_rotate_brute(int arr[],int n,int k)
{
    if (n == 0) return;
    k = k % n;
    if (k > n) return;
    ...
    int temp[k];
    //copy last k element to temp
    int temp_index = 0;
    for(int i=n-k;i<n;i++)
    {
        temp[temp_index++] = arr[i];
    }
    // shift all remaining to the right
    int j = n-1;
    ...
    // append the copied temporary element back to original array
    for(int m = k-1;m>=0;m--)
    {
        arr[j] = temp[m];
        j--;
...
void left_rotate_brute(int arr[],int n,int k)
{
    ...
    int temp[k];
    //copy first k element to temp
    for(int i=0;i<k;i++)
    {
        temp[i] = arr[i];
    }
    // shift all remaining to the left
    int j = 0;
    for(int i=k;i<n;i++)
    {
        arr[j++] = arr[i];
    ...
    // append the copied temporary element back to origanl arrayh
    int temp_index = 0;
    for(;j<n;j++)
    { 
...
void solve() {
    int arr[] = {1,2,3,4,5};
    int k = 2;
    left_rotate_brute(arr,end(arr)-begin(arr), k);
    debug(arr);

    int arr1[] = {1,2,3,4,5};
    left_rotate_optimal(arr1,end(arr1)-begin(arr1), k);
    debug(arr1);
