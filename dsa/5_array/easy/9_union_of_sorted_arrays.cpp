...
vector<int> union_of_sorted_arrays(vector<int>& first,vector<int>& second) {
       
       int i = 0,k=0;
       vector<int> temp;
       while(i< first.size() and k< second.size()) {
            if(first[i] < second[k]){
       ...
       for(int j=k;j<second.size();j++)
       {
        temp.push_back(second[j]);
       }
       return temp;
    }
vector<int> union_of_sorted_arrays_chatgpt(vector<int>& first, vector<int>& second) {
    int i = 0, k = 0;
    vector<int> result;
    result.reserve(first.size() + second.size());  // Preallocate memory to optimize
...
vector<int> union_of_sorted_arrays_using_set_union(vector<int>& first, vector<int>& second) {
    vector<int> result;
    result.reserve(first.size() + second.size());  // Preallocate memory
...
void solve() {
    ...
    auto result = union_of_sorted_arrays_using_set_union(v,v1);
    debug(result);
